...
export const GET: RequestHandler = async ({ url, request }) => {
  ...
  try {
    ...
    const png = await webp_to_png(webp_buffer);

    const node_stream = Readable.from(png.buffer);
    ...
    return new Response(web_stream, {
      ...
      headers: {
        ...
        'Content-Length': String(png.buffer.byteLength),
        'Content-Disposition': `${disposition}; filename="${filename}"`,
        'Cache-Control': `private, max-age=${ms('10mins') / 1000}`
      }
    });
  } catch (err) {
    console.error('convert_webp_to_png failed', err);
    throw error(500, 'Failed to convert WebP to PNG');
  }
};
