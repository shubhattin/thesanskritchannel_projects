import ms from 'ms';
import { REDIS_CACHE_KEYS_CLIENT } from '@tsc/core/redis-keys';
import type { recursive_list_type, shloka_list_type } from '@tsc/core/data-types';
import { get_path_params, type project_type } from '@tsc/core/project-list';
import { get_project_by_key, get_project_info_by_id } from './project-list';
import type { Redis } from '@upstash/redis/cloudflare';
import type { drizzleDbType } from '@tsc/db-schema/db-types';
import { SiteLekhaSchemaZod } from '@tsc/db-schema/schema-zod';
import type { z } from 'zod';
import { and, eq } from 'drizzle-orm';
import { texts, translations } from '@tsc/db-schema/schema';
import { requireProjectPath } from './project-paths-db';
...
const should_use_cache = (options: db_options) => options.isProd ?? false;
...
export const get_site_lekha_data_func = async (
  ...
  let cache = null;
  if (should_use_cache(options)) cache = await redis.get<lekhaType>(cache_key);
