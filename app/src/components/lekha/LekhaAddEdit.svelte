<script lang="ts">
  ...
  const invalidate_lekha_queries = async () => {
    // TODO: 
    await query_client.invalidateQueries({ queryKey: [['site', 'lekha', 'list_lekhas']], exact: false });
    await query_client.invalidateQueries({ queryKey: [['site', 'lekha', 'get_lekha']], exact: false });
  };
