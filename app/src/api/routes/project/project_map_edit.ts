...
} from '~/server/map/path-delete-db.server';
import {
...
import {
  ...
  validateDbPath,
  ROOT_DB_PATH
} from '~/server/map/path-swap';
import { delay } from '~/tools/delay';
import { recursive_list_schema } from '~/state/data_types';
import { countExactPathResources, insertProjectPaths } from '~/server/project/paths-db.server';
import {
  collect_db_paths_from_map,
  validate_explicit_to_add_paths
} from '~/server/map/map-sync.server';
import { TEXT_EDIT_LOCK_NAMESPACE } from '~/server/text/row-edit.server';
import { invalidate_project_caches_effect, runAppEffect, withTransaction } from '~/server/effect';
...
const invalidate_project_caches = async (
  ...
  await Promise.all([
    runAppEffect(invalidate_project_caches_effect(project_id, project_key, redisKeys)),
...
export const update_project_map_route = protectedAdminProcedure
  ...
  .mutation(async ({ input, ctx: { cookie } }) => {
    ...
    const project = await runAppEffect(
      withTransaction('project_map.update', async (tx) => {
        await tx.execute(
        ...
        const toAddPaths = validate_explicit_to_add_paths(
          oldPaths,
          derivedPaths,
          input.to_add_paths
        );
...
const save_project_map_order = protectedAdminProcedure
  ...
  .mutation(async ({ input: { project_id, root_path, edits, map }, ctx: { cookie } }) => {
    ...
    const {
      ...
      withTransaction('project_map.save_order', async (tx) => {
        await tx.execute(
...
const delete_project_map_nodes = protectedAdminProcedure
  ...
  .mutation(async ({ input: { project_id, deleted_paths }, ctx: { cookie } }) => {
    ...
    const { project, map, redisKeys } = await runAppEffect(
      withTransaction('project_map.delete_nodes', async (tx) => {
        await tx.execute(
          sql`select pg_advisory_xact_lock(${TEXT_EDIT_LOCK_NAMESPACE}, ${project_id})`
        );
        const project = await tx.query.projects.findFirst({
...
const get_delete_node_resource_counts = protectedAdminProcedure
  ...
  .query(async ({ input: { project_id, paths } }) => {
    return runAppEffect(
      withTransaction('project_map.delete_node_counts', async (tx) => {
        const project = await tx.query.projects.findFirst({
          where: (tbl, { eq: eqId }) => eqId(tbl.id, project_id),
          columns: { id: true }
        });
