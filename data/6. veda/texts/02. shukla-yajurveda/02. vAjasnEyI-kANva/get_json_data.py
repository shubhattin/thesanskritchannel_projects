...
"""
Convert extracted Shukla-Yajurveda kANva text files into JSON shloka lists.
...
Also updates veda_map.json for:
  shuklayajurvEdaH -> saMhitA -> kANva-vAjasanEyI
...
def _update_shukla_map(*, adhyaya_meta: dict[int, tuple[int, int]]) -> None:
    ...
    target = _find_child(shakha_list, name_nor="kANva-vAjasanEyI")
    if not isinstance(target, dict):
        console.print(
            "[yellow]Could not find kANva-vAjasanEyI node; skipping map update[/]"
        )
        return
...
def main(
    ...
    """
    Walk .txt files under text_data/2/1/1 and generate JSON under data/2/1/1.
    """
