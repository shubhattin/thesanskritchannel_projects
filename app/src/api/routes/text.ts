...
import { notify_site_invalidate_project_map_cache } from '~/server/site/invalidate-project-cache.server';
...
} from '~/server/text/row-edit.server';
import {
  clear_project_memory_caches_effect,
  get_project_by_key_effect,
  get_project_info_by_id_effect,
  get_text_data_effect,
  invalidate_text_edit_redis_effect,
  runAppEffect,
  withDb,
  withTransaction
} from '~/server/effect';
...
const save_text_rows_route = protectedAdminProcedure
  ...
  .mutation(async ({ input: { project_id, selected_text_levels, rows }, ctx: { cookie } }) => {
    ...
    const { redisKeys, mapChanged, projectKey } = await runAppEffect(
      withTransaction('text.save_rows', async (tx) => {
        await tx.execute(
    ...
    if (redisKeys.length > 0) {
      await runAppEffect(invalidate_text_edit_redis_effect(...redisKeys));
    }
    if (mapChanged) {
      await runAppEffect(clear_project_memory_caches_effect(project_id, projectKey));
