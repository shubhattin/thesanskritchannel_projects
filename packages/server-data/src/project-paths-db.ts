...
import type { TxOrDb } from '@tsc/db-schema/db-types';
import { media_attachment, project_paths, texts, translations } from '@tsc/db-schema/schema';

export class ProjectPathNotFoundError extends Error {
  constructor(path: string) {
    super(`Project path not found: ${path}`);
...
export const requireProjectPath = async (
  ...
  if (!row) {
    throw new ProjectPathNotFoundError(path);
  }
  return row;
};
...
export const requireProjectPaths = async (
  ...
  if (missing.length > 0) {
    throw new ProjectPathNotFoundError(missing.join(', '));
  }
  return byPath;
};
